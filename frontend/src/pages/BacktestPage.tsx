...
export function BacktestPage() {
  ...
  const chosen = strategy && result?.cumulative_growth.columns.includes(strategy)
    ? strategy
    : (best?.strategy ?? null);
  ...
  const growth = useMemo(() => {
    ...
    return timeSeries(
      palette,
      [
        { name: `${chosen} · net`, points: series(result.cumulative_growth, chosen), slot: 0 },
        {
          name: `${chosen} · gross`,
      ...
      { format: (v) => num(v, 3), zoom: true, label: "net against gross growth" },
    );
  ...
  const dragChart = useMemo(() => {
    ...
    return bars(palette, rows, { slot: 1, format: (v) => pct(v, 2), label: "cost drag by strategy" });
  }, [result, palette]);

  const columns: Column<BacktestResponse["results"][number]>[] = [
    { key: "s", header: "Strategy", render: (r) => r.strategy },
    { key: "net", header: "Net return", num: true, render: (r) => <Signed value={r.annual_return} render={(v) => pct(v)} /> },
    { key: "gross", header: "Gross", num: true, render: (r) => pct(r.gross_annual_return) },
    { key: "drag", header: "Cost drag", num: true, render: (r) => <span className="neg">{pct(r.cost_drag)}</span> },
    { key: "vol", header: "Volatility", num: true, render: (r) => pct(r.annual_volatility) },
    { key: "sh", header: "Sharpe", num: true, render: (r) => <Signed value={r.sharpe} render={(v) => num(v)} /> },
    { key: "dd", header: "Max drawdown", num: true, render: (r) => <span className="neg">{pct(r.max_drawdown)}</span> },
    { key: "to", header: "Turnover / yr", num: true, render: (r) => pct(r.annual_turnover, 0) },
      ...
      <Card>
        <RunControls showBackend={false}>
          <Field label="Lookback" hint="trading days per fit">
            <input type="number" min={60} step={21} value={lookback} onChange={(e) => setLookback(Number(e.target.value))} />
          </Field>
          <Field label="Rebalance every" hint="trading days">
            <input type="number" min={1} step={1} value={rebalance} onChange={(e) => setRebalance(Number(e.target.value))} />
          </Field>
          <Field label="Cost (bps)" hint="on traded notional">
            <input type="number" min={0} step={1} value={costBps} onChange={(e) => setCostBps(Number(e.target.value))} />
          </Field>
          <button
            type="button"
            className="primary"
        ...
        <>
          ...
          <Card>
            <dl className="grid stats" style={{ margin: 0 }}>
              ...
              <Stat label="Rebalances" value={result.rebalance_dates.length} sub={`every ${result.rebalance_every}d`} />
              <Stat label="Best by Sharpe" value={<span style={{ fontSize: 15 }}>{best?.strategy ?? "--"}</span>} sub={num(best?.sharpe ?? null)} />
              <Stat
                label="Its growth"
          ...
          >
            {growth && <Chart option={growth} className="chart tall" label="net against gross cumulative growth" />}
          </Card>

          <Card title="Cost drag" hint="Gross annual return minus net, by strategy.">
            {dragChart && <Chart option={dragChart} className={barChartClass(result.results.length)} label="cost drag by strategy" />}
          </Card>
