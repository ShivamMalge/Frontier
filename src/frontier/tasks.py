...
Progress is reported through :func:`frontier.jobs.progress.report`, which resolves to
...
from frontier.services import backtest as backtest_service
...
RUN_PIPELINE = "frontier.tasks.run_pipeline"
RUN_BACKTEST = "frontier.tasks.run_backtest"
RUN_INGEST = "frontier.tasks.run_ingest"
...
def run_backtest(request: dict[str, Any]) -> dict[str, Any]:
    ...
    from frontier.api.schemas.backtest import BacktestResponse, BacktestStrategyResult
...
def run_ingest(request: dict[str, Any]) -> dict[str, Any]:
    ...
    from frontier.api.schemas.data import IngestRequest, IngestResponse, Revision
    from frontier.services import market_data
