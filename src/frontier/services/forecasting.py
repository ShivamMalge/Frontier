...
from frontier.errors import InsufficientDataError, UnknownBackendError
from frontier.forecasting.results import SeriesForecast
from frontier.utils.config import MIN_OBSERVATIONS
...
def _keras_lstm(series: pd.Series, params: ForecastParams) -> SeriesForecast:
    ...
    from frontier.forecasting.evaluate_lstm import lstm_predict
    from frontier.forecasting.train_lstm import train_lstm_model
...
def _torch_lstm(prices: pd.DataFrame, params: ForecastParams, on_progress=None):
    from frontier.forecasting.torch_lstm import TorchConfig, forecast_universe
...
def _lightgbm(prices: pd.DataFrame, params: ForecastParams, on_progress=None):
    from frontier.forecasting.gbm import GBMConfig, forecast_universe
