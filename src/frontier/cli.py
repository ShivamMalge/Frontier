...
surface can never drift apart. Installed as the ``frontier`` command; ``python
main.py`` at the repo root runs the same code. ``--help`` for options.
"""
...
from frontier.services.forecasting import available_backends
from frontier.settings import get_settings
from frontier.utils.helpers import ensure_dirs
from frontier.utils.logger import log
...
def main(argv: list[str] | None = None) -> int:
    ...
    for metric in result.forecast_metrics:
        # A flat forecast makes no directional call, so the naive baseline
        # reports null here rather than a misleading 0.5.
        direction = (
            f"{metric.directional_accuracy:5.1%}"
            if metric.directional_accuracy is not None
            else "  n/a"
        )
        log(
            f"  {metric.ticker:6s} MASE={metric.mase_vs_naive:6.3f}  "
            f"dir={direction}  RMSE={metric.rmse:8.3f}"
        )
...
def run() -> None:
    """Console-script entry point (``frontier``)."""
    sys.exit(main())
